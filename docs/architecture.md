...
| sird | Генерация автостереограммы из карты глубины и паттерна | `src/sird/` | чистый JS, ImageData; выполняется в Web Worker |
...
- **depth → sird:** через `DepthMap` (см. `docs/api_contracts.md`) —
  ...
  буфера.
- **sird → ui:** `ImageData` того же размера; `ui` рисует его в canvas и
  экспортирует через `canvas.toBlob`.
...
3. **sird.** Алгоритм Thimbleby, Inglis, Witten («Displaying 3D Images:
   ...
   берёт из паттерна. Режим перекрёстного взгляда = инверсия `z`.
4. **ui.** Слайдеры меняют параметры и перезапускают генерацию (с
   дебаунсом); превью карты глубины показывается рядом для отладки.
...
- Как калибровать E под физический размер пикселя (DPI) — спросить у
  пользователя диагональ экрана или дать ползунок «пока не сойдётся»? Решает
  владелец после первого рабочего MVP. Замеры на стенде показали, что у
  вопроса есть и вторая сторона, не только удобство сведения глазами:
  сепарация округляется до целых пикселей, поэтому число ступеней рельефа
  `≈ E·μ / (2(2−μ)) + 1`. При значениях по умолчанию из
  `docs/api_contracts.md` (`E=90`, `μ=1/3`) ступеней всего 10, и гладкий
  купол читается террасами. E — это не только «удобно/неудобно», но и
  разрешение рельефа, поэтому дефолты контракта, вероятно, придётся
  поднять; конкретные числа владелец выбирает ползунками на стенде.
- Нужна ли стереограмма в реальном времени при вращении (GPU-генератор) —
  см. ADR-003; решается по результатам MVP.
